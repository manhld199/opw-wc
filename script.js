...
function loadData() {
  ...
  apiCall("getMatches").then((data) => {
    ...
    data.forEach((row) => {
      ...
      tbody.innerHTML += `
        <tr>
          <td>${row[0]}</td><td>${row[3]}</td><td>${row[4]} vs ${row[5]}</td>
          <td>${row[12]}</td><td>${row[13]}</td><td>${row[14]}</td><td>${row[15]}</td>
          <td><button class="btn ${betValue === "Cửa trên" ? "selected" : ""}" onclick="bet(this, ${row[0]}, 'Cửa trên')">Cửa trên</button></td>
          <td><button class="btn ${betValue === "Cửa dưới" ? "selected" : ""}" onclick="bet(this, ${row[0]}, 'Cửa dưới')">Cửa dưới</button></td>
        </tr>`;
    });
  });
}
...
function bet(btn, stt, choice) {
  btn.innerText = "⏳...";
  apiCall("submitBet", { stt: stt, choice: choice }).then((res) => {
    showToast(res);
    loadData(); // Tải lại để update nút
  });
}
